...
class UserInterface {
  ...
  getButtons() {
    return document
      .getElementById("available-locations-list")
      .querySelectorAll(".BUTTON-set-location");
  }

  getVisibleButtons() {
    return document
      .getElementById("available-locations-list")
      .querySelectorAll(".BUTTON-set-location:not(.hide)");
  }

  getSelectedButton() {
    return document
      .getElementById("available-locations-list")
      .querySelector(".BUTTON-set-location.selected");
  }

  setupEventListeners() {
    // CLICKS
    this.listen("click", "BUTTON-toggle-atlas-window", () => {
      UI.Atlas.toggle();
    });
    this.listen("click", "BUTTON-close-atlas", () => {
      UI.Atlas.toggle();
    });

    this.listen("click", "BUTTON-toggle-map-window", () => {
      UI.Map.toggle();
    });
    this.listen("click", "BUTTON-close-map", () => {
      UI.Map.toggle();
    });

    this.listen("click", "BUTTON-open-settings", () => {
      UI.Settings.toggle();
      UI.el("location-selection-input").focus();
    });
    this.listen("click", "BUTTON-close-settings", () => {
      UI.Settings.toggle();
    });

    this.listen("click", "BUTTON-toggle-credits-window", () => {
      UI.Credits.toggle();
    });
    this.listen("click", "BUTTON-close-credits", () => {
      UI.Credits.toggle();
    });
    ...
    document.addEventListener("keydown", (event) => {
      ...
      if (UI.Settings.show) {
        ...
        if (
          ...
          // scroll to button
          UI.el("available-locations-list").scroll(
            0,
            this.buttons[this.locationSelectedIndex].offsetTop - 200,
          );
          return;
        }
    ...
    this.el("location-selection-input").addEventListener("input", (event) => {
      ...
      for (const element of buttons) {
        ...
        for (let [index, fragment] of searchFragments.entries()) {
          ...
          found[index] = searchText.toLowerCase().includes(fragment)
            ? true
            : false;
        }
  ...
  #update_setColors() {
    ...
    document
      .querySelector(":root")
      .style.setProperty("--theme-color", colorMain);
    document
      .querySelector(":root")
      .style.setProperty("--theme-color-dark", colorDark);
  ...
  #update_setThemeImage() {
    if (Settings.activeLocation.THEME_IMAGE === null) {
      ...
      Settings.activeLocation.THEME_IMAGE = exists
        ? imgPath
        : Settings.activeLocation.PARENT.THEME_IMAGE;
    }
    const url = `url("${Settings.activeLocation.THEME_IMAGE}")`;
    if (UI.bgElement.style.backgroundImage !== url)
      UI.bgElement.style.backgroundImage = url;
  }
  ...
  #update_setRiseAndSetData() {
    ...
    if (!nextRise) {
      ...
      nextRise = Settings.activeLocation.IS_STAR_RISING_NOW
        ? "- NOW -"
        : convertHoursToTimeString(nextRise * 24, true, false);
    ...
    if (!nextSet) {
      ...
      nextSet = Settings.activeLocation.IS_STAR_SETTING_NOW
        ? "- NOW -"
        : convertHoursToTimeString(nextSet * 24, true, false);
    ...
    if (!nextRise) {
      ...
      const rise = now.setSeconds(
        now.getSeconds() + Settings.activeLocation.NEXT_STAR_RISE * 86400,
      );
    ...
    if (!nextSet) {
      ...
      const set = now.setSeconds(
        now.getSeconds() + Settings.activeLocation.NEXT_STAR_SET * 86400,
      );
  ...
  #update_setIlluminationStatus() {
    ...
    let scDateString = scDate.toLocaleString("default", {
      year: "numeric",
      month: "long",
      day: "numeric",
    });
    UI.setText(
      "illumination-status",
      Settings.activeLocation.ILLUMINATION_STATUS + "\r\n" + scDateString,
    );
  }
  ...
  #update_setDynamicBackgroundColors(brightness, tint, alpha) {
    ...
    if (r.style.getPropertyValue("--bg-brightness") !== brightness)
      r.style.setProperty("--bg-brightness", brightness);
    if (r.style.getPropertyValue("--bg-tint") !== brightness)
      r.style.setProperty("--bg-tint", tint);
    if (r.style.getPropertyValue("--bg-tint-alpha") !== brightness)
  ...
  updateSettingsLocationTimes() {
    ...
    for (let element of buttons) {
      ...
      if (String(location.LOCAL_TIME) === "NaN") {
        ...
        string =
          convertHoursToTimeString(location.LOCAL_TIME / 60 / 60, false, true) +
          "\r\n" +
  ...
  updateDebugUI() {
    ...
    if (bod) {
      ...
      UI.setText(
        "day-length-readable",
        convertHoursToTimeString(bod.ROTATION_RATE),
      );
      UI.setText(
        "hour-length-readable",
        convertHoursToTimeString(bod.ROTATION_RATE / 24),
      );
      UI.setText(
        "current-cycle",
        round(bod.CURRENT_CYCLE(), 3).toLocaleString(),
      );
    ...
    UI.setText(
      "db-local-time",
      convertHoursToTimeString(loc.LOCAL_TIME / 60 / 60),
    );
    ...
    UI.setText(
      "length-of-daylight",
      convertHoursToTimeString(loc.LENGTH_OF_DAYLIGHT * 24, true, false),
    );
    UI.setText(
      "length-of-night",
      convertHoursToTimeString(
    ...
    UI.setText(
      "starrise-time",
      convertHoursToTimeString(loc.LOCAL_STAR_RISE_TIME * 24),
    );
    UI.setText(
      "starset-time",
      convertHoursToTimeString(loc.LOCAL_STAR_SET_TIME * 24),
    );
    UI.setText(
      "next-starrise-countdown",
      round(parseFloat(loc.NEXT_STAR_RISE), 6).toFixed(6),
    );
    UI.setText(
      "next-starset-countdown",
      round(parseFloat(loc.NEXT_STAR_SET), 6).toFixed(6),
    );
    ...
    let next = now.setSeconds(
      now.getSeconds() + loc.NEXT_STAR_RISE * 24 * 60 * 60,
    );
    next = new Date(next).toLocaleString();
    let remain = convertHoursToTimeString(loc.NEXT_STAR_RISE * 24, true, false);
    UI.setText(
      "db-next-starrise",
      (loc.NEXT_STAR_RISE * 24 * 60 * 60).toFixed(0),
    );
    ...
    UI.setText(
      "db-next-starset",
      (loc.NEXT_STAR_SET * 24 * 60 * 60).toFixed(0),
    );
  ...
  setMapLocation(locationName) {
    ...
    if (!location) {
      console.error(
        "Invalid [locationName] parameter passed to [setLocation] function!\nValue passed: " +
          locationName,
      );
      return false;
    }
    ...
    if (this.Map.show) {
      ...
      document.dispatchEvent(
        new CustomEvent("moveMapCameraAboveActiveLocation"),
      );
    }
  ...
  showMapLocationData(location, triggerElement) {
    ...
    UI.setText(
      "map-info-elevation",
      Math.round(location.ELEVATION * 1000, 1).toLocaleString(),
    );
  ...
  #updateMapLocationData() {
    ...
    if (unchanging.includes(UI.mapHoverLocation.ILLUMINATION_STATUS)) {
      ...
      if (UI.mapHoverLocation.IS_STAR_RISING_NOW) {
        ...
        nextRise = convertHoursToTimeString(
          UI.mapHoverLocation.NEXT_STAR_RISE * 24,
          true,
          false,
        );
      }
      ...
      if (UI.mapHoverLocation.IS_STAR_SETTING_NOW) {
        ...
        nextSet = convertHoursToTimeString(
          UI.mapHoverLocation.NEXT_STAR_SET * 24,
          true,
          false,
        );
      }
  ...
  populateAtlasSidebar(system) {
    ...
    for (const star of stars) {
      ...
      const planets = DB.bodies.filter((planet) => {
        if (planet.PARENT && planet.PARENT.NAME === star.NAME) {
          return true;
        }
      });
      ...
      for (const planet of planets) {
        ...
        const moons = DB.bodies.filter((moon) => {
          if (moon.PARENT && moon.PARENT.NAME === planet.NAME) {
            return true;
          }
        });
