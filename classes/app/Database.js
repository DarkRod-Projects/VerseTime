...
class Database {
  ...
  static async createSolarSystems() {
    ...
    for (const sys of systems) {
      Database.#createSolarSystem(sys);
    }
  }
  ...
  static async createCelestialBodies() {
    ...
    for (const body of bodies) {
      Database.#createCelestialBody(body);
    }
  ...
  static #createCelestialBody(data) {
    ...
    let parentBody =
      data.parentBody === "" ? null : getBodyByName(data.parentBody);
    let parentStar =
      data.parentStar === "" ? null : getBodyByName(data.parentStar);
  ...
  static createLocation(data) {
    ...
    let parentBody =
      data.parentBody === "null" ? null : getBodyByName(data.parentBody);
    let parentStar =
      data.parentStar === "null" ? null : getBodyByName(data.parentStar);
  ...
  #sortLocationssByDistance(locations) {
    ...
    for (const loc of locations) {
      ...
      const rise =
        loc.TYPE !== "Asteroid cluster"
          ? Math.round(loc.NEXT_STAR_RISE * 1440)
          : null;
      const set =
        loc.TYPE !== "Asteroid cluster"
          ? Math.round(loc.NEXT_STAR_SET * 1440)
          : null;
